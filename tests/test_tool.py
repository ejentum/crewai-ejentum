"""Unit tests for :class:`crewai_ejentum.EjentumHarnessTool`."""
...
@patch("crewai_ejentum.tool.requests.post")
...
def test_non_string_scaffold_value_is_handled(mock_post, monkeypatch):
    """The scaffold value must be a string; non-string falsy or truthy values fail safe."""
    monkeypatch.setenv("EJENTUM_API_KEY", "test-key")
    mock_post.return_value = _mock_response(
        status_code=200,
    ...
    tool = EjentumHarnessTool()
    result = tool._run(query="anything", mode="reasoning")
